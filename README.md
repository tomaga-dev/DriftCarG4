...
You can change the camera modes in the Godot editor. Klick on Camera in the scene tab, then in the inspector tab you can choose first person or third person. If you choose neither of them, the camera will be third person but with a fixed angle. Race mode makes the camera to rotate according to the car, otherwise it points to the direction the car is moving to (wich may be sideways). A high damping value (e.g. 10) lets the camera rotation follow the cars rotation very tightly.

### Vehicle controller

Klick on monteri in the scene tab. You can set the max force to a higher value (e.g. 30000). This will let the car accelerate much faster. Rev min will set the minimum motor sound frefquency. Rev multiplier will adjust the frequency range of the motor sound. Omega max sets the maximum angular velocity in regular steering mode. Omega max drift sets the maximum angular velocity in drift mode (try higher values to get a feeling for it).
